...
mrouted_common_conf_set() {
    ...
    if [[ "${#values[@]}" -eq 0 ]]; then
        ...
        for i in "${!values[@]}"; do
            mrouted_common_conf_set "$file" "${key[$i]}" "${values[$i]}"
        done
    else
        value="${values[0]}"
...
#   $2 - 值（列表）
mrouted_conf_set() {
    mrouted_common_conf_set "${APP_CONF_DIR}/zoo.cfg" "$@"
}

# 生成默认配置文件
mrouted_generate_conf() {
    # 准备原始默认配置文件或生成空文件
...
mrouted_enable_remote_connections() {
...
# 以后台方式启动应用服务，并等待启动就绪
mrouted_start_server_bg() {
    mrouted_is_server_running && return

    LOG_I "Starting ${APP_NAME} in background..."

	# 使用内置脚本启动服务
    #local start_command="mrouted"
...
# 停止应用服务
mrouted_stop_server() {
    if mrouted_is_server_running ; then
		...
		# 检测停止是否完成
		local counter=10
	    while [[ "$counter" -ne 0 ]] && mrouted_is_server_running; do
